...
module.exports =
class CreatePartitionTables extends EventEmitter
  constructor: (options) ->
    @msg_tag = 'createPartitionTables'
    @options = options
    @templateFile = path.join process.cwd(),'sql','partition-tables.src.sql'

  start: () ->
    if @options[@msg_tag]
      debug @msg_tag, 'create partition tables'
  ...
  onReadFile: (err,data) ->
    if err
      ...
      fs.writeFile path.join( os.tmpdir(),'partition-tables.sql' ),output,(err) => @onWriteFile(err)
  ...
  onWriteFile: (err)->
    ...
    opt =
      cwd: process.cwd()
      env: process.env
    proc = spawn 'psql',['-f', path.join( os.tmpdir(),'partition-tables.sql' ), '-p', @options.port,@options.database], opt
